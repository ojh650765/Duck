...
namespace DuckMow
{
    ...
    public class CurtainField : MaskableGraphic
    {
        ...
        static void Leaf(float u, float v, out float a, out float shade)
        {
            ...
            // Midrib, and a soft gradient across the blade so the shape has a light side.
            float rib = 1f - Edge(0f, 0.045f, Mathf.Abs(u));
            shade = Mathf.Lerp(0.72f, 1.06f, 0.5f + 0.5f * u) - rib * 0.22f;
            // Faint side veins, only strong enough to survive at small sizes as texture.
        ...
        static void Blade(float u, float v, out float a, out float shade)
        {
            ...
            // Bright at the tip where the light catches it, dark in the sward.
            shade = Mathf.Clamp01(Mathf.Lerp(0.55f, 1.12f, t) - 0.18f * Mathf.Abs(u - lean) / Mathf.Max(w, 0.02f));
        }

        /// <summary>Banner cloth: a rounded panel with a woven ripple and a shadowed lower edge.</summary>
        static void Cloth(float u, float v, out float a, out float shade)
        {
            const float r = 0.22f;
            float qx = Mathf.Max(Mathf.Abs(u) - (1f - r), 0f);
            float qy = Mathf.Max(Mathf.Abs(v) - (1f - r), 0f);
            float d = Mathf.Sqrt(qx * qx + qy * qy) - r;
            a = 1f - Edge(-0.02f, 0.015f, d);

            // Two ripples crossing, so a long strip of it reads as hanging fabric rather than card.
            float ripple = 0.09f * Mathf.Sin(u * 6.1f) + 0.06f * Mathf.Sin(v * 4.3f + 1.1f);
            shade = Mathf.Clamp01(0.93f + ripple - 0.16f * Edge(0.2f, 1f, -v));
        }
                     ...
                     int tile, bool solid)
        {
            ...
            // Solid quads sample the dead centre of the cloth tile, which is opaque and flat — the
            // backing must not inherit the tile's rounded corners or the screen edges leak.
            float u0, v0, u1, v1;
            if (solid)
            {
                u0 = 0.5f + 0.19f; v0 = 0.5f + 0.19f;
                u1 = 0.5f + 0.31f; v1 = 0.5f + 0.31f;
            }
            else
            {
